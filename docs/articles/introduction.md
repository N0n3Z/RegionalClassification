# Getting started with nbbbenuts

## Overview

The `nbbbenuts` package converts Belgian geographic codes between
classification systems and across time.

Belgian administrative data references geography in many ways — NIS
commune codes, NUTS regions, postal codes, internal arrondissement codes
— and these change over time as communes merge or district borders
shift. This package unifies all of them.

All classification identifiers are strict uppercase strings
(e.g. `"NIS_MUNICIPALITY_2019"`). See
[`?classification_reference`](https://n0n3z.github.io/regionalclassification/reference/classification_reference.md)
for the full list, or call:
...
Belgian communes are periodically reorganized. Three versions are
supported:

    BEFORE_2019  -->  2019  -->  2025
      591 comm.       583         567

Temporal conversions return a `nature` column describing the type of
change:
...
NUTS 2027 codes for NIS 2025 communes come from the official Statbel
file. For NIS 2019 and BEFORE_2019 communes, NUTS 2027 is derived by
remapping NUTS 2021 codes per EU regulation 2026/195.
...
Some conversions are inherently M:N. The arrondissement of Verviers
(63000) spans two NUTS3 regions: BE335 (francophone) and BE336
(germanophone).
...
For ratio variables (`value_type = "ratio"`), values are replicated
unchanged to each target region. For additive variables (the default),
values are distributed by weight.
...
verifies alignment between your data and a reference classification, or
auto-detects the best match.
...
`n_unknown`, `n_duplicates`, `missing_codes`, `unknown_codes`, and
`status`.
...
diagnose_classification(nuts3_data, "nuts3", master_data)
# Ranks all 23 classifications by match rate and recommends the best one.
```
