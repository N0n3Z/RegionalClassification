...
R package for converting Belgian geographic codes between classification
systems and across time versions.

## Overview

Belgian administrative data uses multiple overlapping classification
systems that change over time. This package provides a unified,
graph-based interface to convert between them — with explicit handling
of ambiguous (M:N or 1:N) mappings.
...
error (`rcl_invalid_classification`).
...
always returns a `data.table` with exactly three columns: `code_from`,
`code_to`, `nature`.

## Conversion Graph

The package uses a BFS-based conversion graph. All paths are resolved
automatically — including multi-hop chains.
...
Belgian communes are reorganised through periodic mergers and
administrative transfers.

    BEFORE_2019 ──[REFNIS_CHANGE_BEFORE2019]──► 2019 ──[REFNIS_CHANGE_2025]──► 2025
       589 communes                               583                             567

The `nature` column in
[`convert_codes()`](https://n0n3z.github.io/regionalclassification/reference/convert_codes.md)
output tracks how each commune changed:
...
NUTS 2027 codes implement EU Regulation 2026/195 (applicable from 1
January 2027). For NIS 2025 communes the mapping uses the official
Statbel file. Key changes for Belgium:
...
> **Note:** There is no direct `NUTS_DISTRICT_2021 ↔︎ NUTS_DISTRICT_2027`
> conversion. Three communes changed province between 2019 and 2025,
> shifting their NUTS3 region. Always route via NIS communes:
> `NUTS_DISTRICT_2021 → NIS_MUNICIPALITY_2019 → NIS_MUNICIPALITY_2025 → NUTS_DISTRICT_2027`.
...
|----|----|
| [`load_master_data()`](https://n0n3z.github.io/regionalclassification/reference/load_master_data.md) | Load the pre-built reference snapshot |
...
| [`list_available_conversions()`](https://n0n3z.github.io/regionalclassification/reference/list_available_conversions.md) | List all supported conversion edges |
| [`get_conversion_matrix()`](https://n0n3z.github.io/regionalclassification/reference/get_conversion_matrix.md) | Full feasibility matrix |
| [`visualize_classification_graph()`](https://n0n3z.github.io/regionalclassification/reference/visualize_classification_graph.md) | Interactive graph (requires visNetwork) |
| [`visualize_conversion_matrix()`](https://n0n3z.github.io/regionalclassification/reference/visualize_conversion_matrix.md) | Matrix view (requires ggplot2) |

## Rebuilding the Data Snapshot

Only needed if the Statbel/Eurostat source files change. Place the files
below in `data/raw/`, then run
...
| `NUTS_ARRONDISSEMENT.csv`          | NUTS 3 ↔︎ internal arrondissement codes |
