...
La fonction centrale du package. Elle retourne un `data.table` à trois colonnes :
...
Toutes les conversions temporelles NIS portent une colonne `nature` qui qualifie
...
Ces valeurs sont utiles pour auditer la stabilité longitudinale d'une série
statistique : une commune `CHANGE_DSTR` ou `CHANGE_PROV` peut changer de
périmètre dans des agrégats supérieurs (arrondissement, province, NUTS2).

### Commune pré-2019 → classification actuelle

```{r before-2019}
# 55022 = Fosses-la-Ville (fusionné en 2019 en commune 58001 = Mettet)
convert_codes("55022", "NIS_MUNICIPALITY_BEFORE_2019", "NIS_MUNICIPALITY_2019", master_data)
convert_codes("55022", "NIS_MUNICIPALITY_BEFORE_2019", "NUTS_DISTRICT_2021",       master_data)
convert_codes("55022", "NIS_MUNICIPALITY_BEFORE_2019", "NUTS_DISTRICT_2027",       master_data)
```
...
convert_codes("1000", "CP",           "NIS_COM_2019", master_data)
convert_codes("1000", "CODE_POSTAL",  "COMMUNE_2019", master_data)
convert_codes("1000", "POSTAL",       "NIS_MUNICIPALITY_2019", master_data)
```
...
Affiche tous les chemins de conversion déclarés avec leur type de relation et
leur **sémantique de périmètre** (colonne `perimeter_relation`).
...
| `overlap`   | La source enjambe plusieurs cibles (ex. Verviers 1:N). Note : `province → region` est désormais `nesting` (N:1) depuis la scission du Brabant |

---

## 4. `check_conversion_path()` et `is_perimeter_preserving()`

`check_conversion_path()` retourne maintenant des informations de périmètre en
plus du test de simplicité.
