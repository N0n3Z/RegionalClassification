...
test_that("check_conversion_path NIS_COMMUNE_2019 -> NUTS3_2021 is simple", {
  ...
  expect_true(CLS_NIS_COMMUNE_2019 %in% r$path)
  expect_true(CLS_NUTS3_2021 %in% r$path)
})
...
test_that("get_all_classification_nodes includes expected nodes", {
  ...
  expect_true(CLS_NIS_COMMUNE_2019 %in% nodes)
  expect_true(CLS_NUTS3_2021       %in% nodes)
  expect_true(CLS_POSTAL           %in% nodes)
  expect_true(CLS_NUTS3_2027       %in% nodes)
})
...
test_that("print_conversion_check prints without error", {
  expect_output(print_conversion_check(CLS_NIS_COMMUNE_2019, CLS_NUTS3_2021))
  expect_output(print_conversion_check(CLS_POSTAL, CLS_NUTS3_2027))
})
...
test_that("check_conversion_path errors for non-canonical identifiers", {
  expect_error(check_conversion_path("NIS_COM_2019", CLS_NUTS3_2021),
               class = "rcl_invalid_classification")
  expect_error(check_conversion_path(CLS_NIS_COMMUNE_2019, "NUTS_2021"),
               class = "rcl_invalid_classification")
})
...
test_that("convert_codes raises rcl_ambiguous_conversion with allow_ambiguous=FALSE", {
  expect_error(
    convert_codes(63000L, CLS_NIS_ARRONDISSEMENT_2019, CLS_NUTS3_2021, master_data,
...
test_that("convert_codes raises rcl_ambiguous_conversion for M:N pair", {
  ...
  expect_error(
    convert_codes(21004L, CLS_NIS_COMMUNE_2019, CLS_NIS_COMMUNE_BEFORE_2019, master_data),
    class = "rcl_ambiguous_conversion"
  )
})
...
test_that("NIS_COMMUNE_2019 -> NIS_COMMUNE_2025 converts without allow_ambiguous", {
  ...
  result <- convert_codes(c(21001L, 21004L), CLS_NIS_COMMUNE_2019, CLS_NIS_COMMUNE_2025,
                           master_data)
...
test_that("NIS_COMMUNE_2025 -> NIS_COMMUNE_2019 is ambiguous (1:N reverse)", {
  r <- check_conversion_path(CLS_NIS_COMMUNE_2025, CLS_NIS_COMMUNE_2019)
  expect_false(r[["is_simple"]])
  expect_equal(r[["relations"]], "1:N")
  # Raises error without allow_ambiguous
  expect_error(
    convert_codes(21001L, CLS_NIS_COMMUNE_2025, CLS_NIS_COMMUNE_2019, master_data),
    class = "rcl_ambiguous_conversion"
  )
})
...
test_that("convert_codes raises rcl_invalid_classification for unknown identifier", {
  expect_error(
    convert_codes(1L, CLS_NIS_COMMUNE_2019, "TOTALLY_UNKNOWN", master_data),
    class = "rcl_invalid_classification"
  )
})
