...
test_that("convert_dataset adds target column correctly", {
  ...
  result <- convert_dataset(dt, "cd_commune", to = CLS_NUTS3_2021, master_data)
...
test_that("convert_dataset auto-detects from when omitted", {
  ...
  expect_no_error(
    convert_dataset(dt, "cd_commune", to = CLS_NUTS3_2021, master_data)
  )
})
...
test_that("convert_dataset raises rcl_invalid_input for missing column", {
  dt <- data.table(x = 1:3)
  expect_error(
    convert_dataset(dt, "nonexistent_col", to = CLS_NUTS3_2021, master_data),
...
test_that("convert_dataset raises rcl_invalid_input for non-data.frame input", {
  expect_error(
    convert_dataset("not_a_table", "col", to = CLS_NUTS3_2021, master_data),
    class = "rcl_invalid_input"
  )
})
...
test_that("convert_dataset works for POSTAL -> NIS_COMMUNE_2019", {
  ...
  result <- convert_dataset(dt, "cp", from = CLS_POSTAL,
                            to = CLS_NIS_COMMUNE_2019, master_data)
  expect_equal(nrow(result), 3L)
