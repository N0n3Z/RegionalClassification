...
# -- Test CD1: conversion simple NIS_MUNICIPALITY_2019 -> NUTS_DISTRICT_2021 ----------------
...
# -- Test CD2: auto-detection de la classification source ----------------------
test_that("convert_dataset auto-detects from when omitted", {
  dt <- data.table(cd_commune = c(21004L, 11002L), pop = c(100L, 200L))
  expect_no_error(
    convert_dataset(dt, "cd_commune", to = CLS_NUTS_DISTRICT_2021, master_data)
  )
})
...
test_that("convert_dataset raises rcl_invalid_input for missing column", {
  dt <- data.table(x = 1:3)
  expect_error(
    convert_dataset(dt, "nonexistent_col", to = CLS_NUTS_DISTRICT_2021, master_data),
...
test_that("convert_dataset raises rcl_invalid_input for non-data.frame input", {
  expect_error(
    convert_dataset("not_a_table", "col", to = CLS_NUTS_DISTRICT_2021, master_data),
    class = "rcl_invalid_input"
  )
})

# -- Test CD5: conversion POSTAL -> NIS_MUNICIPALITY_2019 ---------------------------
test_that("convert_dataset works for POSTAL -> NIS_MUNICIPALITY_2019", {
  dt <- data.table(cp = c(1000L, 2000L, 4000L))
  result <- convert_dataset(dt, "cp", from = CLS_POSTAL,
                            to = CLS_NIS_MUNICIPALITY_2019, master_data)
  expect_equal(nrow(result), 3L)
  expect_true(any(grepl("nis|commune", names(result), ignore.case = TRUE)))
})

# -- Test CD6: donnees inchangees (colonnes supplementaires preservees) ---------
