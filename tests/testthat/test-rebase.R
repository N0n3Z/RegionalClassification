...
test_that("rebase_series sums values for fused communes", {
  result <- rebase_series(
    ...
    version_map = setNames(list(2022L, 2025L), c(CLS_NIS_MUNICIPALITY_2019, CLS_NIS_MUNICIPALITY_2025)),
...
test_that("rebase_series returns data unchanged when source == target", {
  ...
  result <- rebase_series(
    ...
    code_col    = "commune",
    value_cols  = "pop",
    version_map = setNames(list(2025L), CLS_NIS_MUNICIPALITY_2025),
    to          = CLS_NIS_MUNICIPALITY_2025,
    master_data = master_data
  )
...
test_that("rebase_series aggregates multiple value columns", {
  ...
  result <- rebase_series(
    ...
    version_map = setNames(list(2022L), CLS_NIS_MUNICIPALITY_2019),
    to          = CLS_NIS_MUNICIPALITY_2025,
    master_data = master_data
  )
...
test_that("split_weights_template returns empty table when no ambiguous codes exist", {
  expect_message(
    tpl <- split_weights_template(CLS_NIS_MUNICIPALITY_2019, CLS_NIS_DISTRICT_2019, master_data),
...
test_that("split_weights_template errors for invalid classification", {
  expect_error(
    split_weights_template("MAUVAISE_CLASSIF", CLS_NUTS_DISTRICT_2021, master_data),
    class = "rcl_invalid_classification"
  )
})
