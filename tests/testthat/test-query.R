...
test_that("validate_codes returns TRUE for known codes, FALSE for unknowns", {
  result <- validate_codes(c(21004L, 99999L, 11002L), CLS_NIS_MUNICIPALITY_2019, master_data)
...
test_that("validate_codes returns data.table with character codes", {
  result <- validate_codes(c("1000", "9999"), CLS_POSTAL, master_data)
...
test_that("validate_codes works for NUTS_DISTRICT_2021", {
  result <- validate_codes(c("BE100", "ZZZZ"), CLS_NUTS_DISTRICT_2021, master_data)
  expect_true(result[code == "BE100", is_valid])
  expect_false(result[code == "ZZZZ", is_valid])
})

test_that("validate_codes works for NBB_DISTRICT_2021", {
  result <- validate_codes(c("21", "99"), CLS_NBB_DISTRICT_2021, master_data)
...
test_that("get_label returns French names for NIS_MUNICIPALITY_2019", {
  result <- get_label(c(21004L, 11002L), CLS_NIS_MUNICIPALITY_2019, master_data, lang = "fr")
  expect_true(is.data.table(result))
...
test_that("get_label returns Dutch names when lang = 'nl'", {
  result <- get_label(21004L, CLS_NIS_MUNICIPALITY_2019, master_data, lang = "nl")
  expect_equal(result$label, "Brussel")
})

test_that("get_label returns NA for NUTS2 (no names in data)", {
  result <- get_label(c("BE10", "BE21"), CLS_NUTS_PROVINCE_2021, master_data)
  expect_equal(nrow(result), 2L)
  expect_true(all(is.na(result$label)))
})

test_that("get_label returns NA for unknown code", {
  result <- get_label(c(21004L, 99999L), CLS_NIS_MUNICIPALITY_2019, master_data)
  expect_false(is.na(result[code == "21004", label]))
  expect_true(is.na(result[code == "99999", label]))
})

test_that("get_label works for NUTS_DISTRICT_2021", {
  result <- get_label("BE100", CLS_NUTS_DISTRICT_2021, master_data, lang = "fr")
  expect_false(is.na(result$label))
})

test_that("get_label works for POSTAL", {
  result <- get_label(1000L, CLS_POSTAL, master_data, lang = "fr")
...
test_that("get_crosswalk returns a data.table with named columns", {
  result <- get_crosswalk(CLS_NIS_MUNICIPALITY_2019, CLS_NUTS_DISTRICT_2021, master_data)
  expect_true(is.data.table(result))
  expect_true(CLS_NIS_MUNICIPALITY_2019 %in% names(result))
  expect_true(CLS_NUTS_DISTRICT_2021       %in% names(result))
})

test_that("get_crosswalk covers all source codes", {
  result <- get_crosswalk(CLS_NIS_MUNICIPALITY_2019, CLS_NUTS_DISTRICT_2021, master_data)
  all_2019 <- nbbbenuts:::.list_codes_for(CLS_NIS_MUNICIPALITY_2019, master_data)
  # Every source code is represented (some may map to NA if no NUTS available)
  expect_true(all(all_2019 %in% result$NIS_MUNICIPALITY_2019))
})

test_that("get_crosswalk for POSTAL -> NIS_MUNICIPALITY_2019 is N:1", {
  result <- get_crosswalk(CLS_POSTAL, CLS_NIS_MUNICIPALITY_2019, master_data)
...
test_that("get_crosswalk errors for invalid classification", {
  expect_error(
    get_crosswalk(CLS_NIS_MUNICIPALITY_2019, "INCONNU", master_data),
    class = "rcl_invalid_classification"
  )
})
...
test_that("get_crosswalk weights=TRUE adds weight column with value 1 for simple pair", {
  result <- get_crosswalk(CLS_NIS_MUNICIPALITY_2019, CLS_NUTS_DISTRICT_2021, master_data, weights = TRUE)
...
test_that("get_crosswalk weights=TRUE uses equal weights for ambiguous pair", {
  clear_split_weights()
  result <- get_crosswalk(CLS_NIS_DISTRICT_2019, CLS_NUTS_DISTRICT_2021, master_data, weights = TRUE)
  expect_true("weight" %in% names(result))
  verviers <- result[NIS_DISTRICT_2019 == "63000"]
...
test_that("get_crosswalk weights=TRUE uses registered weights when available", {
  ...
  result <- get_crosswalk(CLS_NIS_DISTRICT_2019, CLS_NUTS_DISTRICT_2021, master_data, weights = TRUE)
  verviers <- result[NIS_DISTRICT_2019 == "63000"]
  expect_equal(verviers[NUTS_DISTRICT_2021 == "BE335", weight], 0.857, tolerance = 1e-9)
  expect_equal(verviers[NUTS_DISTRICT_2021 == "BE336", weight], 0.143, tolerance = 1e-9)
  clear_split_weights()
})

test_that("get_crosswalk weights=FALSE produces no weight column", {
  result <- get_crosswalk(CLS_NIS_MUNICIPALITY_2019, CLS_NUTS_DISTRICT_2021, master_data, weights = FALSE)
