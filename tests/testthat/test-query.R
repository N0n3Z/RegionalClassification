...
test_that("validate_codes returns TRUE for known codes, FALSE for unknowns", {
  result <- validate_codes(c(21004L, 99999L, 11002L), CLS_NIS_COMMUNE_2019, master_data)
...
test_that("validate_codes works for NUTS3_2021", {
  result <- validate_codes(c("BE100", "ZZZZ"), CLS_NUTS3_2021, master_data)
...
test_that("validate_codes works for INTERNAL_ARRONDISSEMENT", {
  result <- validate_codes(c("21", "99"), CLS_INTERNAL_ARRONDISSEMENT, master_data)
...
test_that("get_label returns French names for NIS_COMMUNE_2019", {
  result <- get_label(c(21004L, 11002L), CLS_NIS_COMMUNE_2019, master_data, lang = "fr")
  expect_true(is.data.table(result))
...
test_that("get_label returns Dutch names when lang = 'nl'", {
  result <- get_label(21004L, CLS_NIS_COMMUNE_2019, master_data, lang = "nl")
  expect_equal(result$label, "Brussel")
})

test_that("get_label returns NA for NUTS2 (no names in data)", {
  result <- get_label(c("BE10", "BE21"), CLS_NUTS2_2021, master_data)
  expect_equal(nrow(result), 2L)
  expect_true(all(is.na(result$label)))
})

test_that("get_label returns NA for unknown code", {
  result <- get_label(c(21004L, 99999L), CLS_NIS_COMMUNE_2019, master_data)
...
test_that("get_label works for NUTS3_2021", {
  result <- get_label("BE100", CLS_NUTS3_2021, master_data, lang = "fr")
...
test_that("get_crosswalk returns a data.table with named columns", {
  result <- get_crosswalk(CLS_NIS_COMMUNE_2019, CLS_NUTS3_2021, master_data)
  expect_true(is.data.table(result))
  expect_true(CLS_NIS_COMMUNE_2019 %in% names(result))
  expect_true(CLS_NUTS3_2021       %in% names(result))
})

test_that("get_crosswalk covers all source codes", {
  result <- get_crosswalk(CLS_NIS_COMMUNE_2019, CLS_NUTS3_2021, master_data)
  all_2019 <- nbbbenuts:::.list_codes_for(CLS_NIS_COMMUNE_2019, master_data)
...
test_that("get_crosswalk for POSTAL -> NIS_COMMUNE_2019 is N:1", {
  result <- get_crosswalk(CLS_POSTAL, CLS_NIS_COMMUNE_2019, master_data)
...
test_that("get_crosswalk errors for invalid classification", {
  expect_error(
    get_crosswalk(CLS_NIS_COMMUNE_2019, "INCONNU", master_data),
    class = "rcl_invalid_classification"
  )
})
...
test_that("get_crosswalk weights=TRUE adds weight column with value 1 for simple pair", {
  result <- get_crosswalk(CLS_NIS_COMMUNE_2019, CLS_NUTS3_2021, master_data, weights = TRUE)
...
test_that("get_crosswalk weights=TRUE uses equal weights for ambiguous pair", {
  clear_split_weights()
  result <- get_crosswalk(CLS_NIS_ARRONDISSEMENT_2019, CLS_NUTS3_2021, master_data, weights = TRUE)
  expect_true("weight" %in% names(result))
...
test_that("get_crosswalk weights=FALSE produces no weight column", {
  result <- get_crosswalk(CLS_NIS_COMMUNE_2019, CLS_NUTS3_2021, master_data, weights = FALSE)
