...
Checklist d'actions pour finaliser la branche `check_relation`
(détails et explications dans `CHECK_RELATION.md`).

> ⚠️ Pré-requis : R installé + `install.packages("readxl")`.
> Tant que ces étapes ne sont pas faites, la suite de tests est **rouge**
> (les `.rds` livrés contiennent encore l'ancien code province `20000`).
